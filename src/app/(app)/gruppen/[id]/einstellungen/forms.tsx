    ...
    <form action={formAction} className="space-y-4">
      ...
      <label className="flex items-start gap-3 text-sm">
        ...
        <span>
          ...
          <span className="hint block">{t("Reduziert die nötigen Überweisungen auf ein Minimum.")}</span>
        </span>
      </label>
...
export function ArchiveForm({ groupId, archived }: { groupId: string; archived: boolean }) {
  ...
  return (
    <form action={formAction} className="space-y-2">
      ...
      <SubmitButton className="btn-secondary" pendingLabel="…">
        {archived ? t("Aus dem Archiv holen") : t("Gruppe archivieren")}
      </SubmitButton>
      <FormAlert state={state} />
    </form>
  );
  ...
  return (
    <form action={formAction} className="space-y-2">
      ...
      <label className="label" htmlFor="guest-name">
        {t("Person ohne Konto hinzufügen")}
      </label>
      <div className="flex flex-wrap gap-2">
        <input
          id="guest-name"
      ...
      <p className="hint">
        {t(
          "Für alle, die kein Konto anlegen möchten. Sie zählen bei Ausgaben und Salden ganz normal mit und können später von einem echten Konto übernommen werden.",
        )}
      </p>
      <FormAlert state={state} />
    </form>
    ...
    <form action={formAction} className="space-y-3">
      ...
      <div>
        ...
        <input
          ...
          maxLength={80}
          className="input"
          defaultValue={t("{name} (Fortsetzung)", { name: groupName })}
        />
      </div>
      <label className="flex items-start gap-3 text-sm">
        <input
        ...
        />
        <span>
          <span className="font-medium">{t("Diese Gruppe danach archivieren")}</span>
          <span className="hint block">
            {t("Sie bleibt vollständig erhalten, steht aber nicht mehr in der Liste.")}
          </span>
        </span>
      </label>
      <FormAlert state={state} />
      <SubmitButton className="btn-secondary" pendingLabel={t("Wird übertragen …")}>
        {t("Fortsetzung anlegen")}
      </SubmitButton>
    </form>
  );
}
  ...
  return (
    <form action={formAction} className="space-y-2">
      ...
      <SubmitButton className={enabled ? "btn-secondary" : "btn-primary"} pendingLabel="…">
        {enabled ? t("Gemeinsamen Link abschalten") : t("Gemeinsamen Link erstellen")}
      </SubmitButton>
      <FormAlert state={state} />
    </form>
  );
